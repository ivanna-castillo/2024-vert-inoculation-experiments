...
immersiontime <- read.csv("immersiontime_long_clean.csv")

#creating a column with dpi
immersiontime$dpi <- as.Date(immersiontime$date, format = "%d/%m/%Y") - as.Date("22/03/2024", format = "%d/%m/%Y")

#making trt a factor
immersiontime$trt <- as.factor(immersiontime$trt)
...
plot2 <- immersiontime %>%
  ...
  labs(x = "DPI", y = "% Chlorosis", title = "Immersion Time", color = "")
