###In this script I will plot the data for the spore concentration
...
severity_mean <- aggregate(as.matrix(immersiontime_wide[,3:8]) ~ immersiontime_wide$trt, FUN=mean) 

#making a basic color palette for each treatment
colors = c("red", "blue", "green")

#the layout plot
immersiontimeplot <- plot(0, type='n', xlim=range(dpi), ylim=c(0,100), ylab='% Chlorosis', xlab='DPI', 
                               main = "Immersion Time", cex = 0.9) #Make empty plot
...
legend(x = 17,
       y = 40, legend = severity_mean$`immersiontime_wide$trt`,
