#making the spore concentration experiment's plot using ggplot2
...
sporeconcentration <- read.csv("sporeconcentration_long_clean.csv")

#creating a column with dpi
sporeconcentration$dpi <- as.Date(sporeconcentration$date, format = "%d/%m/%Y") - as.Date("22/03/2024", format = "%d/%m/%Y")

#making trt a factor
sporeconcentration$trt <- as.factor(sporeconcentration$trt)

#making the plot with ggplot, aes makes the grid and it is where we determine the looks of the graph
plot1 <- sporeconcentration %>%
  ...
  labs(x = "DPI", y = "% Chlorosis", title = "Spore Concentration", color = "")
